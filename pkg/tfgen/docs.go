...
func (p *tfMarkdownParser) parse() (entityDocs, error) {
	...
	// Convert examples.
	p.ret.Description = p.g.convertExamples(p.ret.Description, true)
	for _, arg := range p.ret.Arguments {
	...
	for attrName, attrDoc := range p.ret.Attributes {
		p.ret.Attributes[attrName] = p.g.convertExamples(attrDoc, false)
	}
